    ...
    def run(self):
        ...
        try:
            ...
            for side_band in side_bands:
                ...
                sis2.set_bias_voltage_iterative(sis_voltage_2)
                for step_freq, freq_range in enumerate(inter_frequencies_reshaped, 1):
                    ...
                    for if_channel in if_channels.keys():
                        ...
                        for fi, freq in enumerate(freq_range):
                            ...
                            logger.info(
                                f"TT freq {test_tone_freq / 1e9:.4f}GHz IF {freq/1e9:.4f}GHz power {power:.4f} dBm"
                            )
                            send_to_telegram(
...
def main():
    ...
    p1 = win.addPlot()
    p1.setTitle(
        f"SRR measure LO {lo_frequency/1e9:.0f} GHz", color="#413C58", size="10pt"
    )
