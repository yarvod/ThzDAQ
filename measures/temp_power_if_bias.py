...
temp_contr = TemperatureController(port=7777)
sis = SisBlock()
sis.connect()

bias_t_voltages = np.linspace(0, 9e-3, 10)
...
try:
    ...
    for voltage in resistor_voltages:
        ...
        for bias in bias_t_voltages:
            sis.set_bias_voltage(bias)
            time.sleep(0.5)
            get_bias = sis.get_bias_voltage()
            da = {
                "bias_voltage": get_bias,
                "power": [],
                "freq": [],
            }
            logger.info(f"Set Bias Voltage {bias}")
            send_to_telegram(f"Set Bias Voltage {bias}")
            ...
            for freq in freqs:
                ...
                da["power"].append(nrx.get_power())
                da["freq"].append(_freq)
            dat["data"].append(da)
